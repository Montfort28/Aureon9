...
function StatusPill({ label }: { label: string }) {
  ...
  return <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-medium ${styles[label] || "bg-slate-100 text-slate-700 border-slate-200"}`}>{label}</span>;
}
...
export default function Aureon9AdminSettingsAnalyticsDashboard() {
                ...
                <CardContent>
                  <Tabs defaultValue="channels" className="w-full">
                    ...
                    <TabsContent value="templates" className="mt-5">
                      ...
                      <div className="mt-4 grid gap-4 md:grid-cols-[260px_1fr]">
                        <div className="rounded-2xl border border-slate-200 p-4">
                          <p className="font-semibold">Template Selector</p>
                          <div className="mt-4 space-y-3">
                            <Select>
                              ...
                              <SelectContent>
                                {templateRows.map((row) => (
                                  <SelectItem key={`${row.code}-${row.channel}`} value={`${row.code}-${row.channel}`}>{row.code} · {row.channel}</SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                            <Button variant="outline" className="w-full rounded-2xl border-slate-200">Load Template</Button>
                          </div>
                        </div>
                        <div className="rounded-2xl border border-slate-200 p-4">
                          <div className="grid gap-3 md:grid-cols-2">
                            <Input className="rounded-2xl border-slate-200" defaultValue="AUREON9: Verification Approved" />
                            <Input className="rounded-2xl border-slate-200" defaultValue="REVIEW_APPROVED" />
                          </div>
                          <Textarea className="mt-3 min-h-[180px] rounded-2xl border-slate-200" defaultValue={`Hello {{member_name}},\n\nYour review case {{reference_id}} has been approved.\nYour verification level is now: {{verification_level}}.\n\nPowered By ODIEBOARD`} />
                          <div className="mt-3 flex gap-3">
                            <Button className="rounded-2xl bg-[#0A2540] hover:bg-[#14385f]">Save Template</Button>
                            <Button variant="outline" className="rounded-2xl border-slate-200">Preview Render</Button>
                          </div>
                        </div>
                      </div>
                    </TabsContent>
                    ...
                    <TabsContent value="timers" className="mt-5 space-y-4">
                      ...
                      <div className="rounded-2xl border border-slate-200 p-4">
                        <p className="font-semibold">High-Risk Auto-Escalation Policy</p>
                        <p className="mt-2 text-sm text-slate-500">Automatically escalate cases when participant class or requested verification level falls inside controlled capital-grade categories.</p>
                        <div className="mt-4 grid gap-3 md:grid-cols-3">
                          <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3"><span className="text-sm">Capital Participants</span><Switch defaultChecked /></div>
                          <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3"><span className="text-sm">Strategic Partners</span><Switch defaultChecked /></div>
                          <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3"><span className="text-sm">Institutional Participants</span><Switch defaultChecked /></div>
                        </div>
                      </div>
                    </TabsContent>
                        ...
                        <Card className="rounded-2xl border-slate-200">
                          <CardContent className="p-4">
                            ...
                            <div className="mt-4 grid gap-3">
                              ...
                              <Input className="rounded-2xl border-slate-200" defaultValue="Mark failed after final attempt" />
                            </div>
                          </CardContent>
                        </Card>
                        <Card className="rounded-2xl border-slate-200">
                          <CardContent className="p-4">
                            <p className="font-semibold">Governance Delivery Guardrails</p>
                            <div className="mt-4 space-y-3 text-sm text-slate-600">
                              <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3"><span>Approval emails only after transaction commit</span><Switch defaultChecked /></div>
                              <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3"><span>Escalation copies to executive desk</span><Switch defaultChecked /></div>
                              <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3"><span>Preserve rejection notes in notification payload</span><Switch defaultChecked /></div>
                            </div>
                          </CardContent>
                        </Card>
                      </div>
                    ...
                    <div className="space-y-4">
                      {queueAgingRows.map((row) => (
                        <div key={row.band}>
                          ...
                          <div className="mt-1">{statusPillInline(row.status)}</div>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <Card className="rounded-2xl border-slate-200 shadow-sm">
                ...
                <CardContent>
                  <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <Button variant="outline" className="rounded-2xl border-slate-200">
                      <SlidersHorizontal className="mr-2 h-4 w-4" /> Custom KPI Set
                    </Button>
                  </div>
                      ...
                      <TableBody>
                        {eventAnalytics.map((row) => (
                          <TableRow key={row.event}>
                            ...
                            <TableCell>
                              <div className="flex items-center gap-3">
                                <div className="w-28"><Progress value={row.rate} className="h-2" /></div>
                  ...
                  {[
                    {
                      ...
                      text: "Approval, rejection, and document request events show the highest reliance on email due to auditability and formal tone.",
                    },
                    {
                      icon: MessageSquare,
                      title: "In-app alerts are strongest for reviewer execution speed",
                      text: "Reviewer reminders and queue movements are best surfaced through persistent in-app notifications and dashboard counters.",
                    },
                    {
                      icon: Workflow,
                      title: "Escalation aging is the most sensitive KPI",
                      text: "Cases older than 48 hours should trigger visible executive oversight in capital, strategic partner, and institutional participant classes.",
                    },
                  ].map((item) => {
                    const Icon = item.icon;
                    return (
            ...
            <section>
              <Card className="rounded-2xl border-slate-200 shadow-sm">
                <CardHeader>
                  ...
                  <CardDescription>Recent controlled actions affecting templates, channel rules, timers, and notification governance.</CardDescription>
                </CardHeader>
                <CardContent>
...
function statusPillInline(label: string) {
  ...
  return <span className={`text-xs font-medium ${color[label] || "text-slate-500"}`}>{label}</span>;
}
